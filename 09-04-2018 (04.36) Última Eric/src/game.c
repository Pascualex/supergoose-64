...
STATUS game_create_from_file(Game *game, char *file_name) {
    ...
    if (game_reader_load_spaces(game, file_name) == ERROR) {
        return ERROR;
    }

    if (game_reader_load_objects(game, file_name) == ERROR) {
        return ERROR;
    }

    if (game_reader_load_links(game, file_name) == ERROR) {
        return ERROR;
    }

    if (game_reader_load_players(game, file_name) == ERROR) {
        return ERROR;
    }
...
STATUS game_add_space(Game *game, Space *space) {
    ...
    if (game == NULL || space == NULL || game->spaces_number >= MAX_SPACES || space_get_id(space) <= SPACE_BASE_ID || space_get_id(space) >= SPACE_BASE_ID + ID_RANGE) {
        return ERROR;
    }
...
STATUS game_add_object(Game *game, Object *object, Id location_id) {
    ...
    if (game == NULL || object == NULL || game->objects_number >= MAX_OBJECTS || location_id == NO_ID || object_get_id(object) <= OBJECT_BASE_ID || object_get_id(object) >= OBJECT_BASE_ID + ID_RANGE) {
        return ERROR;
    }
...
STATUS game_add_link(Game *game, Link *link) {
    ...
    if (game == NULL || link == NULL || game->links_number >= MAX_LINKS
            || link_get_id(link) <= LINK_BASE_ID || link_get_id(link) >= LINK_BASE_ID + ID_RANGE) {
        return ERROR;
    }
...
STATUS game_add_player(Game *game, Player *player, Id location_id) {
    ...
    if (game == NULL || player == NULL || game->players_number >= MAX_PLAYERS
            || player_get_id(player) <= PLAYER_BASE_ID || player_get_id(player) >= PLAYER_BASE_ID + ID_RANGE) {
        return ERROR;
    }
    ...
    if (game_set_player_location(game, location_id, player_get_id(player)) == ERROR) {
        return ERROR;
    }
...
STATUS game_set_object_location(Game *game, Id location_id, Id object_id) {
    ...
    if (game == NULL || location_id == NO_ID || object_id == NO_ID) {
        return ERROR;
    }
    ...
    if (object == NULL || new_holder == NULL) {
        return ERROR;
    }
...
STATUS game_set_player_location(Game *game, Id location_id, Id playerId) {
    ...
    if (game == NULL || location_id == NO_ID || location_id <= SPACE_BASE_ID || location_id >= SPACE_BASE_ID + ID_RANGE || playerId == NO_ID) {
        return ERROR;
    }
...
void *game_find_name(Game *game, char *name) {
    ...
    if (game == NULL || name == NULL || strcmp(name, "NO_INFO") == 0) {
        return NULL;
    }
...
Link *game_get_connection(Game *game, Space *origin, Space *destination) {
    ...
    if (game == NULL || origin == NULL || destination == NULL) {
        return NULL;
    }
...
STATUS game_callback_grasp(Game *game, char *string) {
    ...
    if (game == NULL || game->players == NULL || game->objects == NULL || string == NULL) {
        return ERROR;
    }

    if (atoi(string) > 0 && atoi(string) < ID_RANGE) {
    ...
    if (player_get_location(game->players[0]) == NO_ID || space_check_object((Space *) game_find(game, player_get_location(game->players[0])), object_get_id(object)) == FALSE || player_is_full(game->players[0]) == TRUE || player_get_location(game->players[0]) != object_get_location(object)) {
        return ERROR;
    }

    if (game_set_object_location(game, player_get_id(game->players[0]), object_get_id(object)) == ERROR) {
        return ERROR;
    }
...
STATUS game_callback_drop(Game *game, char *string) {
    ...
    if (player_get_location(game->players[0]) == NO_ID || space_is_full((Space *) game_find(game, player_get_location(game->players[0]))) == TRUE || player_is_empty(game->players[0]) == TRUE || player_check_object(game->players[0], object_get_id(object)) == FALSE) {
        return ERROR;
    }

    if (game_set_object_location(game, player_get_location(game->players[0]), object_get_id(object)) == ERROR) {
        return ERROR;
    }
...
STATUS game_callback_check(Game *game, char *string) {
    ...
    if (game == NULL || game->players == NULL || game->objects == NULL || string == NULL || atoi(string) < 0 || atoi(string) > ID_RANGE) {
        return ERROR;
    }
