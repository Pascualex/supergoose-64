...
STATUS set_add(Set *set, Id id) {
    ...
    if (set == NULL || id == NO_ID || set_is_full(set) == TRUE) return ERROR;
...
STATUS set_del(Set *set, Id id) {
    ...
    if (set == NULL || id == NO_ID || set_is_empty(set) == TRUE) return ERROR;

    id_position = set_get_id_position(set, id);

    if (id_position == NOT_FOUND) return ERROR;
...
Id set_get_id(Set *set, int id_position) {
    ...
    if (set == NULL || id_position < 0 || id_position >= set->ids_number) return NO_ID;
...
int set_get_id_position(Set *set, Id id) {
    ...
    if (set == NULL || id == NO_ID) return NOT_FOUND;
